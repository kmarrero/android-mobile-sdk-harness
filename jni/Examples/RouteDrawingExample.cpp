...
void RouteDrawingExample::Update(float dt)
{
    ...
    if(!m_createdRoutes)
    {
        ...
        //
        std::vector<RouteVertex> points = builder.Start(routeRed, halfWidth, linkSpeedMetersPerSecond)
        ...
        std::vector<RouteVertex> otherPoints = builder.Start(routeBlue, halfWidth/2.f, linkSpeedMetersPerSecond)
        ...
        std::vector<RouteVertex> islandCircuitPoints = builder.Start(routeGreen, halfWidth, linkSpeedMetersPerSecond)
