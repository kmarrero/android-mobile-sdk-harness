...
void SearchExample::Start()
{
    ...
    EXAMPLE_LOG("Attempting to search for %s at (%f, %f)!\n",
              query.c_str(),
    ...
    {
    	EXAMPLE_LOG("Attempt to search for %s POI failed!\n", query.c_str());
    }
}
...
void SearchExample::HandlePoiSearchResults(const Eegeo::Search::Service::PoiSearchResult& result)
{
    if(result.success)
    {
            ...
            ++ it)
        {
            ...
            EXAMPLE_LOG("Search discovered place with name %s and place ID %s at (%f, %f)\n",
                      entry.name.c_str(),
                      entry.placeId.c_str(),
