...
if [[ $1 == --merge ]]; then
	...
	touch ./.temp.empty
	mv ./libs/platform/AppOnMap.cpp ./AppOnMap_LATEST.cpp
	git merge-file ./jni/AppOnMap.cpp ./.temp.empty ./AppOnMap_LATEST.cpp
	echo "Counted $? conflicts between current and canonical AppOnMap.cpp"
	rm -f ./AppOnMap_LATEST.cpp
	rm -r ./.temp.empty

	touch ./.temp.empty
	mv ./libs/platform/AppOnMap.h ./AppOnMap_LATEST.h
	git merge-file ./jni/AppOnMap.h ./.temp.empty ./AppOnMap_LATEST.h
	...
	rm -f ./libs/platform/AppOnMap.cpp
	rm -f ./libs/platform/AppOnMap.h
fi